...
spark = (
        ...
        .appName("VotesProcessingJob")
        .config('spark.jars.packages', 'org.apache.spark:spark-sql-kafka-0-10_2.12:3.5.6')
        .config('spark.sql.streaming.checkpointLocation', DataLakeConfig.PATH_CHECKPOINT)
        .config('spark.sql.streaming.stateStore.stateStoreDir', DataLakeConfig.PATH_STATES)
...
seats_data = seats_data = [
    ("Madrid", 37), ("Barcelona", 32), ("Valencia", 16),
    ("Sevilla", 12), ("Alicante", 12), ("Málaga", 11),
    ("Murcia", 10), ("Cádiz", 9), ("Vizcaya", 8),
    ("La Coruña", 8), ("Islas Baleares", 8), ("Las Palmas", 8),
    ("Asturias", 7), ("Santa Cruz de Tenerife", 7), ("Zaragoza", 7),
    ("Granada", 7), ("Pontevedra", 7), ("Córdoba", 6),
    ("Tarragona", 6), ("Gerona", 6), ("Guipúzcoa", 6),
    ("Toledo", 6), ("Badajoz", 6), ("Jaén", 5), ("Almería", 5),
    ("Navarra", 5), ("Castellón", 5), ("Cantabria", 5),
    ("Valladolid", 5), ("Ciudad Real", 5), ("Huelva", 5),
    ("León", 4), ("Lérida", 4), ("Cáceres", 4),
    ("Albacete", 4), ("Burgos", 4), ("Salamanca", 4),
    ("Lugo", 4), ("Orense", 4), ("La Rioja", 4),
    ("Álava", 4), ("Guadalajara", 3), ("Huesca", 3),
    ("Cuenca", 3), ("Zamora", 3), ("Ávila", 3),
    ("Palencia", 3), ("Segovia", 3), ("Teruel", 3),
    ("Soria", 2), ("Ceuta", 1), ("Melilla", 1)
]

df_seats_ref = spark.createDataFrame(seats_data, ["province_name", "total_seats"])


def compute_seats_from_votes(df_batch, batch_id):
    """"""
    if df_batch.isEmpty():
        return
    ...
    df_batch_with_totals = df_batch.withColumn(
        "total_votes_province",
        F.sum("votes_per_party").over(window_totals)
    )
    ...
    df_dhondt = df_valids.join(F.broadcast(df_seats_ref), on="province_name")

    # explode: generating dividers
    df_quotients = df_dhondt.withColumn(
        "divisor",
        F.explode(F.sequence(F.lit(1), F.col("total_seats")))
    ).withColumn(
        "quotient",
        F.col("votes_per_party") / F.col("divisor")
    )

    # ranking and selection. Making sure of the 350 limit
    # ordering by quotient, using votes_per_party for breaking the tie
    window_ranking = Window.partitionBy("province_name") \
        .orderBy(F.desc("quotient"), F.desc("votes_per_party"))

    # computing who gets the seat
    df_winners = df_quotients \
    ...
    df_candidates = df_valids.select("province_name", "political_party_norm").distinct()

    # making LEFT JOIN. If it is not on winners, seats will be null -> setting it to 0
    df_final_result = df_candidates \
        .join(df_winners, on=["province_name", "political_party_norm"], how="left") \
        .fillna({"seats": 0})
    ...
    df_kafka_message.write \
        .format("kafka") \
        .option("kafka.bootstrap.servers", KafkaConfiguration.KAFKA_BROKER_DOCKER) \
        .option("topic", KafkaConfiguration.TOPIC_VOTES_SEATS_PROVINCES) \
        .save()
...
def run_job():
    ...
    df_votes_from_kafka_no_duplicated_id = (
        df_raw_votes_from_kafka
        .dropDuplicates(["id"])
        .select(
            "id",
    ...
    df_votes_processed = (
        df_votes_from_kafka_no_duplicated_id.withColumn(
    ...
    df_votes_processed_with_doc_id = (
        df_votes_processed
        .withColumn(
            "doc_id",
            F.sha2(F.concat_ws(
    ...
    stream_votes_silver = spark.readStream \
        .schema(vote_schema_norm) \
        .format("parquet") \
    ...
    df_votes_aggregated = stream_votes_silver \
        .groupBy("province_name", "political_party_norm") \
        .count() \
    ...
    stream_seats = (
        df_votes_aggregated
        .writeStream
        .outputMode("complete")  # sending all information updated on each trigger
        .foreachBatch(compute_seats_from_votes)
        .trigger(processingTime="1 minute")
        .option("checkpointLocation", DataLakeConfig.CHECKPOINT_GOLD_PARTIES_PROVINCES_SEATS)
        .start()
    )
